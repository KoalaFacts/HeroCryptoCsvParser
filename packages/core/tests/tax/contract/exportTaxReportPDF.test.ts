    ...
    it("should have exportTaxReportPDF function available", () => {
      ...
      expect(() => {
        const exportTaxReportPDF =
          require("../../../src/tax/exportTaxReportPDF")
  ...
  describe("Input Validation Contract", () => {
    it("should validate required TaxReport parameter", async () => {
      try {
        ...
        await expect(exportTaxReportPDF(null as never)).rejects.toThrow();

        // Test with undefined report
        await expect(exportTaxReportPDF(undefined as never)).rejects.toThrow();
    ...
    it("should handle optional PDFExportOptions parameter", async () => {
      ...
      try {
        ...
        const resultWithOptions = await exportTaxReportPDF(
          mockReport,
          createMockPDFOptions(),
        );
    ...
    it("should validate PDFExportOptions template values", async () => {
      ...
      try {
        ...
        const validTemplates: PDFExportOptions["template"][] = [
          "STANDARD",
          "DETAILED",
          "SUMMARY",
        ];
        ...
        const invalidOptions = { template: "INVALID" } as never;
        await expect(
  ...
  describe("Output Contract Validation", () => {
    ...
    it("should include transaction details when requested", async () => {
      ...
      try {
        ...
        const detailedResult = await exportTaxReportPDF(
          mockReport,
          optionsWithDetails,
        );
    ...
    it("should generate different output for different templates", async () => {
      ...
      try {
        ...
        const standardResult = await exportTaxReportPDF(mockReport, {
          template: "STANDARD",
        });
        const detailedResult = await exportTaxReportPDF(mockReport, {
          template: "DETAILED",
        });
        const summaryResult = await exportTaxReportPDF(mockReport, {
          template: "SUMMARY",
    ...
    it("should include optimization strategies when requested", async () => {
      ...
      try {
        ...
        expect(withOptimization.length).toBeGreaterThan(
          withoutOptimization.length,
        );
