...
describe("T012: Contract Test - initializeStorage Function", () => {
  ...
  describe("Function Interface Contract", () => {
    it("should have initializeStorage function available", () => {
      ...
      expect(() => {
        const initializeStorage = require("../../../src/tax/initializeStorage")
  ...
  describe("Input Validation Contract", () => {
    ...
    it("should validate maxCacheSize is positive", async () => {
      try {
        ...
        await expect(
          initializeStorage(configWithNegativeCache),
        ).rejects.toThrow(/cache.*size|positive|invalid/i);
        ...
        await expect(initializeStorage(configWithZeroCache)).rejects.toThrow(
          /cache.*size|positive|invalid/i,
        );
    ...
    it("should validate indexedFields array", async () => {
      try {
        ...
        await expect(
          initializeStorage(configWithInvalidIndexes),
        ).rejects.toThrow(/indexed.*fields|invalid/i);
  ...
  describe("Error Handling Contract", () => {
    ...
    it("should handle storage initialization failures", async () => {
      try {
        ...
        await expect(initializeStorage(problematicConfig)).rejects.toThrow(
          /initialization.*failed|storage.*error|permission/i,
        );
    ...
    it("should handle invalid encryption key format", async () => {
      try {
        ...
        await expect(initializeStorage(invalidKeyConfig)).rejects.toThrow(
          /encryption.*key|invalid.*key/i,
        );
  ...
  describe("Storage Adapter Integration Contract", () => {
    ...
    it("should maintain storage adapter state consistency", async () => {
      ...
      try {
        ...
        expect(afterCacheStats.cacheSize).toBeGreaterThanOrEqual(
          initialStats.cacheSize,
        );

        await adapter.clearCache();

        const afterClearStats = await adapter.getStorageStats();
        expect(afterClearStats.cacheSize).toBeLessThanOrEqual(
          afterCacheStats.cacheSize,
        );
