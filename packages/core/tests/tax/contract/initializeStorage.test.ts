    ...
    it("should have StorageAdapter interface structure", () => {
      ...
      expect(() => {
        const MockStorageAdapter =
          require("../../../src/tax/storage/StorageAdapter")
            .StorageAdapter as new () => StorageAdapter;
  ...
  describe("Analytics and Aggregations Contract", () => {
    it("should implement getTransactionsByDateRange", async () => {
      ...
      try {
        ...
        for (const transaction of result) {
          expect(transaction.originalTransaction.timestamp).toBeInstanceOf(
            Date,
          );
          expect(
            transaction.originalTransaction.timestamp.getTime(),
    ...
    it("should implement getTransactionsByAsset", async () => {
      ...
      try {
        ...
        for (const transaction of result) {
          ...
          const hasAsset =
            transaction.originalTransaction.originalData?.asset === asset ||
            transaction.originalTransaction.taxEvents.some(
              (event) => event.amount?.asset.symbol === asset,
            );
  ...
  describe("Error Handling Contract", () => {
    it("should handle invalid transaction data gracefully", async () => {
      ...
      try {
        ...
        await expect(adapter.batchInsert([invalidTransaction])).rejects.toThrow(
          /invalid|required|transaction/i,
        );
    ...
    it("should handle storage capacity limits", async () => {
      ...
      const manyTransactions = Array.from({ length: 10000 }, () =>
