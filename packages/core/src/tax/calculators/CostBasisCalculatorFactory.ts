...
import { createFIFOCalculator, type FIFOCalculator } from "./FIFOCalculator";
import {
  createSpecificIdentificationCalculator,
  type LotIdentifier,
  type SpecificIdentificationCalculator,
...
/**
 * Options for cost basis calculation
 */
 ...
 */
export function calculateCostBasisWithMethod(
  method: CostBasisMethod,
  disposal: Transaction,
  acquisitions: Transaction[],
 ...
 */
export function getSupportedMethods(): CostBasisMethod[] {
 ...
 */
export function isMethodSupported(method: string): method is CostBasisMethod {
  return getSupportedMethods().includes(method as CostBasisMethod);
}

/**
 * Get default cost basis method
 */
export function getDefaultMethod(): CostBasisMethod {
  return "FIFO";
}
 ...
 */
export function getMethodDescription(method: CostBasisMethod): string {
  switch (method) {
    case "FIFO":
...
export function validateOptions(
  ...
  switch (method) {
    ...
    case "SPECIFIC_IDENTIFICATION":
      if (!options?.lotIdentifiers) {
        errors.push(
          "lotIdentifiers is required for Specific Identification method",
        );
...
export function createCostBasisCalculator(
  ...
  return createCalculator(method);
}
...
export function calculateCostBasis(
  ...
  return calculateCostBasisWithMethod(method, disposal, acquisitions, options);
}
