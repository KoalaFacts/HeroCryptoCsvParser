import { Transaction, TransactionType, SpotTrade, Transfer, StakingDeposit, StakingWithdrawal, StakingReward, Fee, Interest, Airdrop, Swap, LiquidityAdd, LiquidityRemove } from '../types/transactions';
...
export class CsvExporter {
  ...
  private transactionToRow(transaction: Transaction): CsvRow {
    ...
    switch (transaction.type) {
      ...
      case 'INTEREST':
        ...
        row.description = `${interest.context?.protocol || ''} ${interest.context?.period || ''}`.trim();
        break;
      ...
      case 'LIQUIDITY_ADD':
        ...
        row.amount = liquidityAdd.assets.map(a => a.amount.toString()).join('+');
      ...
      case 'LIQUIDITY_REMOVE':
        ...
        row.amount = liquidityRemove.assets.map(a => a.amount.toString()).join('+');
   ...
   */
  private formatLocation(location?: { platform?: string; address?: string; label?: string }): string {
    if (!location) return '';
    if (location.address) return location.address;
    if (location.label) return `${location.platform || 'unknown'}:${location.label}`;
  ...
  private rowsToCSV(rows: any[]): string {
    ...
    const fields = this.options.fields.length > 0 
      ? this.options.fields 
      : Object.keys(rows[0]);
