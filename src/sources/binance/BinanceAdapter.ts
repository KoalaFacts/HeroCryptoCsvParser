...
export class BinanceAdapter extends SourceAdapter<BinanceTransactionRecord> {
  ...
  protected convertRecord(record: BinanceTransactionRecord, options?: ConversionOptions): Transaction {
    ...
    if (process.env.NODE_ENV === 'test' && categorization.type === 'UNKNOWN') {
      console.log(`Unknown operation: "${record.operation}" - Remark: "${record.remark}"`);
    }
    ...
    switch (categorization.type) {
      ...
      case 'TRANSFER':
        const direction = categorization.subType === 'deposit' ? 'IN' : 
                         categorization.subType === 'withdrawal' ? 'OUT' : 
                         'INTERNAL';
      ...
      case 'STAKING_WITHDRAWAL':
        return this.createStakingWithdrawal(id, timestamp, record, changeAmount);
  ...
  private createTransfer(
    ...
    return {
      ...
      direction,
      transferType: direction === 'IN' ? 'deposit' : direction === 'OUT' ? 'withdrawal' : 'internal',
      from: direction === 'IN' ? { platform: 'external' } : { platform: 'binance', label: record.account },
      to: direction === 'OUT' ? { platform: 'external' } : { platform: 'binance', label: record.account }
    };
  }
  ...
  private createSwap(
    ...
    return {
      ...
      originalData: {
        ...
        swapType: record.operation.includes('Small Assets') ? 'dust' : 'instant'
      }
    } as Swap;
  }
  ...
  private determineFeeType(operation: string, remark: string): 'trading' | 'network' | 'platform' | 'other' {
    const op = operation.toLowerCase();
    const rmk = remark.toLowerCase();
    if (op.includes('trading') || rmk.includes('trading')) return 'trading';
    if (op.includes('withdraw') || op.includes('network') || rmk.includes('network')) return 'network';
