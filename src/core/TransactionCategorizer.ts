...
export class TransactionCategorizer {
    ...
    for (const mapping of this.mappings) {
      if (this.matches(searchText, mapping.pattern)) {
        return {
          ...
          confidence: mapping.priority ? Math.min(1, mapping.priority / 100) : 0.8,
          matchedRule: mapping
        };
      }
    }
  ...
  private loadDefaultMappings(): void {
    const defaults: OperationMapping[] = [
      ...
      { pattern: /\b(buy|purchase|bought)\b/i, type: 'SPOT_TRADE', subType: 'buy', priority: 50 },
      { pattern: /\b(sell|sold)\b/i, type: 'SPOT_TRADE', subType: 'sell', priority: 50 },
      { pattern: /\bdeposit\b/i, type: 'TRANSFER', subType: 'deposit', priority: 40 },
      { pattern: /\bwithdraw(al)?\b/i, type: 'TRANSFER', subType: 'withdrawal', priority: 40 },
      { pattern: /\bstaking[\s-]?purchase\b/i, type: 'STAKING_DEPOSIT', priority: 60 },
      { pattern: /\bstaking[\s-]?redemption\b/i, type: 'STAKING_WITHDRAWAL', priority: 60 },
      { pattern: /\bstaking[\s-]?rewards?\b/i, type: 'STAKING_REWARD', priority: 60 },
      { pattern: /\b(trading[\s-]?)?fee\b/i, type: 'FEE', subType: 'trading', priority: 30 },
      { pattern: /\bcommission\b/i, type: 'FEE', subType: 'commission', priority: 30 },
      { pattern: /\bconvert\b/i, type: 'SWAP', subType: 'instant', priority: 45 },
      { pattern: /\bswap\b/i, type: 'SWAP', priority: 45 },
      { pattern: /\bliquid(ity)?[\s-]?swap[\s-]?add\b/i, type: 'LIQUIDITY_ADD', priority: 55 },
      { pattern: /\bliquid(ity)?[\s-]?swap[\s-]?remove\b/i, type: 'LIQUIDITY_REMOVE', priority: 55 },
      ...
      { pattern: /\bmining[\s-]?revenue\b/i, type: 'INTEREST', subType: 'mining', priority: 35 },
      { pattern: /\breferral\b/i, type: 'INTEREST', subType: 'referral', priority: 35 },
      
      // Coinbase mappings
      { pattern: /\breceive\b/i, type: 'TRANSFER', subType: 'receive', priority: 40 },
      ...
      // DeFi mappings
      { pattern: /\bprovide[\s-]?liquidity\b/i, type: 'LIQUIDITY_ADD', priority: 55 },
      { pattern: /\bremove[\s-]?liquidity\b/i, type: 'LIQUIDITY_REMOVE', priority: 55 },
      { pattern: /\byield[\s-]?farm/i, type: 'INTEREST', subType: 'farming', priority: 35 },
      { pattern: /\bharvest\b/i, type: 'INTEREST', subType: 'harvest', priority: 35 },
      { pattern: /\bclaim\b/i, type: 'INTEREST', subType: 'claim', priority: 30 },
 ...
 */
export function createCategorizer(mappings?: OperationMapping[]): TransactionCategorizer {
