import { BatchEntryRecordOptions } from './BatchEntryRecordOptions';
...
export class CsvBatchEntryRecordOptions implements BatchEntryRecordOptions {
  ...
  validateFieldDefinitions(descriptors: FieldDescriptor<any>[], recordName: string): void {
    const indices = descriptors.map(d => d.definition.fieldLocator.fieldIndex);
  ...
  readLine(line: string, descriptors: FieldDescriptor<any>[]): string[] {
    ...
    for (const descriptor of descriptors) {
      ...
      if (descriptor.definition.fieldLocator.fieldPositionFrom !== undefined &&
