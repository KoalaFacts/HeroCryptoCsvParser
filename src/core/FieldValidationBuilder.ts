...
import { FieldValidatorContext } from './FieldContext';

export class FieldValidationBuilder<TRecord, TProperty = any> {
  private validationRules: Array<(context: FieldValidatorContext<TRecord>) => ValidationResult<TRecord>> = [];
  ...
  when(
    ...
    conditionalRules: (builder: FieldValidationBuilder<TRecord, TProperty>) => void
  ): this {
    const conditionalBuilder = new FieldValidationBuilder<TRecord, TProperty>(this.fieldName, this.errorCode);
  ...
  unless(
    ...
    return this.when((record, value) => !predicate(record, value), conditionalRules);
  }
    ...
    this.validationRules.push((context) => {
      ...
      if (value !== expectedValue) {
        return ValidationResult.failure({
          ...
          message: errorMessage || `Field '${this.fieldName}' must equal ${expectedValue}`,
          field: this.fieldName,
          value
        });
      }
    ...
    this.validationRules.push((context) => {
      ...
      if (value === forbiddenValue) {
        return ValidationResult.failure({
          ...
          message: errorMessage || `Field '${this.fieldName}' must not equal ${forbiddenValue}`,
          field: this.fieldName,
          value
        });
      }
  ...
  must(
    predicate: ((value: TProperty) => boolean) | ((value: TProperty, record: TRecord) => boolean),
    errorMessage?: string
  ): this {
    this.validationRules.push((context) => {
      const value = context.currentValue as TProperty;
      const isValid = predicate.length === 1
        ? (predicate as (value: TProperty) => boolean)(value)
        : (predicate as (value: TProperty, record: TRecord) => boolean)(value, context.record);
      
      if (!isValid) {
        return ValidationResult.failure({
          code: this.errorCode || 'MUST',
          message: errorMessage || `Field '${this.fieldName}' validation failed`,
          field: this.fieldName,
          value
        });
      }
  ...
  mustNot(
    ...
    this.validationRules.push((context) => {
      const value = context.currentValue as TProperty;
      const isForbidden = predicate.length === 1
        ? (predicate as (value: TProperty) => boolean)(value)
        : (predicate as (value: TProperty, record: TRecord) => boolean)(value, context.record);
    ...
    this.validationRules.push((context) => {
      ...
      if (!regex.test(value)) {
        return ValidationResult.failure({
          ...
          message: errorMessage || `Field '${this.fieldName}' must match pattern ${regex}`,
          field: this.fieldName,
          value
        });
      }
    ...
    this.validationRules.push((context) => {
      ...
      if (value.length > max) {
        return ValidationResult.failure({
          ...
          message: errorMessage || `Field '${this.fieldName}' must be at most ${max} characters`,
          field: this.fieldName,
          value
        });
      }
    ...
    this.validationRules.push((context) => {
      ...
      if (value.length !== length) {
        return ValidationResult.failure({
          ...
          message: errorMessage || `Field '${this.fieldName}' must be exactly ${length} characters`,
          field: this.fieldName,
          value
        });
      }
    ...
    this.validationRules.push((context) => {
      ...
      if (isNaN(value) || value < minValue) {
        return ValidationResult.failure({
          ...
          message: errorMessage || `Field '${this.fieldName}' must be at least ${minValue}`,
          field: this.fieldName,
          value: context.currentValue
        });
      }
    ...
    this.validationRules.push((context) => {
      ...
      if (isNaN(value) || value > maxValue) {
        return ValidationResult.failure({
          ...
          message: errorMessage || `Field '${this.fieldName}' must be at most ${maxValue}`,
          field: this.fieldName,
          value: context.currentValue
        });
      }
    ...
    this.validationRules.push((context) => {
      ...
      if (isNaN(value) || value < min || value > max) {
        return ValidationResult.failure({
          ...
          message: errorMessage || `Field '${this.fieldName}' must be between ${min} and ${max}`,
          field: this.fieldName,
          value: context.currentValue
        });
      }
    ...
    this.validationRules.push((context) => {
      ...
      if (!allowedValues.includes(value)) {
        return ValidationResult.failure({
          ...
          message: errorMessage || `Field '${this.fieldName}' must be one of: ${allowedValues.join(', ')}`,
          field: this.fieldName,
          value
        });
      }
  ...
  oneOfEnum<E extends Record<string, any>>(enumType?: E, errorMessage?: string): this {
    this.validationRules.push((context) => {
      const value = context.currentValue;
      ...
      if (!enumValues.includes(value)) {
        return ValidationResult.failure({
          ...
          message: errorMessage || `Field '${this.fieldName}' must be a valid enum value`,
          field: this.fieldName,
          value
        });
      }
  ...
  build(): (context: FieldValidatorContext<TRecord>) => ValidationResult<TRecord> {
    return (context) => {
