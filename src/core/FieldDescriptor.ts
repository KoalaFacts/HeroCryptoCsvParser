...
export class FieldLocator {
  ...
  isFixedWidth(): boolean {
    return this.fieldPositionFrom !== undefined && this.fieldPositionTo !== undefined;
  }
}
...
export class FieldDescriptor<TRecord, TProperty = any> {
  ...
  public setter?: (context: FieldGetterContext<TRecord>, value: TProperty) => void;
  public mapper?: (context: FieldMapperContext<TRecord>) => ValidationResult<TRecord>;
  public validator?: (context: FieldValidatorContext<TRecord>) => ValidationResult<TRecord>;
  ...
  setWith(setter: (context: FieldGetterContext<TRecord>, value: TProperty) => void): this {
    this.setter = setter;
    return this;
  }
  ...
  mapWith(
    ...
    this.mapper = (context) => {
      try {
        ...
        if (this.setter) {
          const getterContext = new FieldGetterContext(context.fieldName, context.record);
        ...
        return ValidationResult.failure<TRecord>({
          ...
          message: errorMessage || (error instanceof Error ? error.message : 'Mapping failed'),
  ...
  validateWith(
    rules: (builder: FieldValidationBuilder<TRecord, TProperty>) => FieldValidationBuilder<TRecord, TProperty>
  ): this {
    const builder = new FieldValidationBuilder<TRecord, TProperty>(this.definition.name);
  ...
  static builder<T, P>(definition: FieldDefinition): FieldDescriptorBuilder<T, P> {
