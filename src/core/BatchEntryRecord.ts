...
import { FieldDescriptor, FieldDefinition, FieldLocator } from './FieldDescriptor';
import { FieldGetterContext, FieldMapperContext, FieldFormatterContext, FieldValidatorContext } from './FieldContext';
import { BatchEntryRecordOptions } from './options/BatchEntryRecordOptions';
import { CsvBatchEntryRecordOptions } from './options/CsvBatchEntryRecordOptions';

export abstract class BatchEntryRecord<TRecord extends BatchEntryRecord<TRecord>> {
  ...
  public getFieldDescriptors(): ReadonlyArray<FieldDescriptor<TRecord>> {
    if (!this._fieldDescriptors) {
      this._fieldDescriptors = this._fieldBuilders
        .sort((a, b) => {
      ...
      if (this._fieldDescriptors.length === 0) {
        throw new Error(`No fields defined for ${this.constructor.name}. Use 'fieldFor' to define fields.`);
      }
      ...
      if (names.length !== uniqueNames.size) {
        throw new Error(`Field names must be unique in ${this.constructor.name}.`);
      }
      
      const options = this.getOptions();
      options.validateFieldDefinitions(this._fieldDescriptors as FieldDescriptor<any>[], this.constructor.name);
    }
  ...
  public getRecordHeaders(): string[] {
    if (!this._recordHeaders) {
      ...
      this._recordHeaders = descriptors
        .sort((a, b) => a.definition.fieldLocator.fieldIndex - b.definition.fieldLocator.fieldIndex)
  ...
  protected fieldFor<TProperty>(
    ...
    const definition = new FieldDefinition(fieldName, new FieldLocator(fieldIndex));
    ...
    descriptor.mapper = (ctx: FieldMapperContext<TRecord>) => {
      ...
      try {
        ...
        if (descriptor.setter) {
          const getterContext = new FieldGetterContext(ctx.fieldName, ctx.record);
  ...
  protected fieldForWithPosition<TProperty>(
    ...
    const locator = new FieldLocator(fieldIndex, fieldPositionFrom, fieldPositionTo);
