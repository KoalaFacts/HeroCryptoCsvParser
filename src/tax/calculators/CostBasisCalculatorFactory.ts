...
export class CostBasisCalculatorFactory {
	...
	static isMethodSupported(method: string): method is CostBasisMethod {
		return CostBasisCalculatorFactory.getSupportedMethods().includes(
	 ...
	 */
	static getRecommendedMethod(jurisdictionCode: string): CostBasisMethod {
		switch (jurisdictionCode) {
			...
			default:
				return CostBasisCalculatorFactory.getDefaultMethod();
		}
	}
