...
export class SpecificIdentificationCalculator {
  ...
  calculateCostBasis(
    ...
    if (difference > 0.000001) { // Allow for floating point precision
      throw new Error(
    ...
    const eligibleLots = availableAcquisitions
      ...
      .map(acq => ({
        ...
        holdingPeriod: Math.floor(
          (disposalDate.getTime() - getTransactionTimestamp(acq).getTime()) / (1000 * 60 * 60 * 24)
        )
      }));
...
export function calculateSpecificIdentificationCostBasis(
  ...
  return calculator.calculateCostBasis(disposal, lotIdentifiers, availableAcquisitions);
}
