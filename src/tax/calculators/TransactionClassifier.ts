...
export class TransactionClassifier {
	...
	classifyTransaction(context: ClassificationContext): TransactionTaxTreatment {
		...
		const classification = this.getDetailedClassification(
			transaction,
			eventType,
		);
		...
		const isCgtEligible = this.isCGTEligible(
			transaction,
			eventType,
			isPersonalUse,
		);
		...
		const treatment: TransactionTaxTreatment = {
			...
			treatmentReason: this.buildTreatmentReason(
				eventType,
				classification,
				isPersonalUse,
			),
	...
	private isIncomeEvent(transaction: Transaction): boolean {
		...
		const incomeKeywords = ["staking", "reward", "airdrop", "interest"];
