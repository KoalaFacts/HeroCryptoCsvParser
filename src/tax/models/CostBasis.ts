...
export class AcquisitionLotModel implements AcquisitionLot {
  ...
  public withRemainingAmount(newRemainingAmount: number): AcquisitionLotModel {
    if (newRemainingAmount < 0 || newRemainingAmount > this.amount) {
      throw new Error('New remaining amount must be between 0 and original amount');
    }
...
export class CostBasisModel implements CostBasis {
  ...
  public static createFIFO(
    ...
    for (const lot of sortedLots) {
      ...
      const amountFromThisLot = Math.min(remainingToDispose, lot.remainingAmount);
    ...
    const holdingPeriod = this.calculateHoldingPeriod(earliestDate, disposalDate);
  ...
  public static createSpecificIdentification(
    ...
    const holdingPeriod = this.calculateHoldingPeriod(new Date(earliestDate), disposalDate);
   ...
   */
  private static calculateHoldingPeriod(acquisitionDate: Date, disposalDate: Date): number {
    const msPerDay = 24 * 60 * 60 * 1000;
    return Math.ceil((disposalDate.getTime() - acquisitionDate.getTime()) / msPerDay);
  }
  ...
  private validateInput(data: CostBasis): void {
    ...
    if (!data.method || !['FIFO', 'SPECIFIC_IDENTIFICATION'].includes(data.method)) {
      throw new Error('Method must be either FIFO or SPECIFIC_IDENTIFICATION');
    }

    if (!data.acquisitionDate || !(data.acquisitionDate instanceof Date) || isNaN(data.acquisitionDate.getTime())) {
      throw new Error('Acquisition date must be a valid Date object');
    }

    if (typeof data.acquisitionPrice !== 'number' || data.acquisitionPrice < 0) {
    ...
    if (Math.abs(data.totalCost - expectedTotalCost) > 0.01) { // Allow for small floating point differences
      throw new Error(
    ...
    const lotsTotal = data.lots.reduce((sum, lot) => sum + (lot.amount * lot.unitPrice), 0);
  ...
  public validate(): { isValid: boolean; errors: string[]; warnings: string[] } {
    ...
    if (this.holdingPeriod > 10000) { // ~27 years
    ...
    if (this.method === 'FIFO') {
      ...
      for (let i = 0; i < sortedByDate.length - 1; i++) {
        if (new Date(originalOrder[i].date) > new Date(originalOrder[i + 1].date)) {
