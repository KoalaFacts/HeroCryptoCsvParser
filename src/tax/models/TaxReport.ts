...
export class TaxReportModel implements TaxReport {
  ...
  constructor(data: TaxReport) {
    ...
    this.optimizationStrategies = data.optimizationStrategies ? [...data.optimizationStrategies] : undefined;
    this.metadata = {
      ...data.metadata,
  ...
  private validateInput(data: TaxReport): void {
    ...
    if (!data.generatedAt || !(data.generatedAt instanceof Date)) {
      throw new Error('Generated at date is required and must be a Date object');
    }
  ...
  private validateMetadata(metadata: TaxReport['metadata']): void {
    if (typeof metadata.totalTransactions !== 'number' || metadata.totalTransactions < 0) {
    ...
    if (typeof metadata.generationTime !== 'number' || metadata.generationTime < 0) {
   ...
   */
  private validateTransactionsInPeriod(transactions: TaxableTransaction[], taxPeriod: TaxPeriod): void {
  ...
  public getTransactionsByAsset(asset: string): TaxableTransaction[] {
    return this.transactions.filter(transaction => {
      ...
      if ('baseAsset' in originalTx && originalTx.baseAsset?.asset?.symbol === asset) {
        return true;
      }

      if ('quoteAsset' in originalTx && originalTx.quoteAsset?.asset?.symbol === asset) {
        return true;
      }
  ...
  public getTransactionsByExchange(exchange: string): TaxableTransaction[] {
    return this.transactions.filter(transaction =>
      transaction.originalTransaction.source.name.toLowerCase() === exchange.toLowerCase()
    );
  }
  ...
  public getTransactionsByTreatment(eventType: string): TaxableTransaction[] {
    return this.transactions.filter(transaction =>
      transaction.taxTreatment.eventType === eventType
    );
  }
  ...
  public hasOptimizationStrategies(): boolean {
    return this.optimizationStrategies !== undefined && this.optimizationStrategies.length > 0;
  }
  ...
  public getOptimizationStrategiesByPriority(): TaxStrategy[] {
    ...
    return [...this.optimizationStrategies].sort((a, b) => a.priority - b.priority);
  }
  ...
  public toJSON(): TaxReport {
    return {
      ...
      optimizationStrategies: this.optimizationStrategies ? [...this.optimizationStrategies] : undefined,
      metadata: {
        ...this.metadata,
