...
export class TaxPeriodModel implements TaxPeriod {
  ...
  private validateInput(data: TaxPeriod): void {
    ...
    if (periodDays > 731) { // Allow up to 2 years + 1 day for leap years
  ...
  public getElapsedDays(): number {
    ...
    return Math.floor((now.getTime() - this.startDate.getTime()) / (24 * 60 * 60 * 1000));
  }
   ...
   */
  public getMonths(): Array<{ year: number; month: number; name: string; startDate: Date; endDate: Date }> {
    const months: Array<{ year: number; month: number; name: string; startDate: Date; endDate: Date }> = [];
    const monthNames = [
    ...
    while (currentDate <= this.endDate) {
      ...
      const startDate = monthStartDate > this.startDate ? monthStartDate : this.startDate;
