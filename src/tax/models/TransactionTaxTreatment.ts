...
export class TransactionTaxTreatmentModel implements TransactionTaxTreatment {
	...
	public static createIncome(
		...
		return new TransactionTaxTreatmentModel({
			...
			cgtDiscountApplied: false,
			treatmentReason:
				"Transaction generates ordinary income taxable at marginal tax rates",
			applicableRules,
		});
	}
	...
	public static createDeductible(
		...
		return new TransactionTaxTreatmentModel({
			...
			cgtDiscountApplied: false,
			treatmentReason:
				"Transaction creates deductible expense or adds to cost base",
			applicableRules,
		});
	}
	 ...
	 */
	public static createStakingRewardIncome(
		applicableRules: TaxRule[] = [],
	): TransactionTaxTreatmentModel {
		return TransactionTaxTreatmentModel.createIncome(
	 ...
	 */
	public static createAirdropIncome(
		applicableRules: TaxRule[] = [],
	): TransactionTaxTreatmentModel {
		return TransactionTaxTreatmentModel.createIncome(
			"Airdrop Income",
	 ...
	 */
	public static createDeFiYieldIncome(
		applicableRules: TaxRule[] = [],
	): TransactionTaxTreatmentModel {
		return TransactionTaxTreatmentModel.createIncome(
			"DeFi Yield Income",
	 ...
	 */
	public static createTradingFeeDeduction(
		applicableRules: TaxRule[] = [],
	): TransactionTaxTreatmentModel {
		return TransactionTaxTreatmentModel.createDeductible(
	...
	public static createSpotTradeDisposal(
		...
		return TransactionTaxTreatmentModel.createDisposal(
			"Spot Trade Disposal",
			isPersonalUse,
			cgtDiscountApplied,
			applicableRules,
	...
	public static createSpotTradeAcquisition(
		...
		return TransactionTaxTreatmentModel.createAcquisition(
	...
	private validateInput(data: TransactionTaxTreatment): void {
		...
		if (data.cgtDiscountApplied && !data.isCgtEligible) {
			throw new Error(
				"CGT discount can only be applied to CGT eligible transactions",
			);
		}
		...
		if (
			data.eventType === "ACQUISITION" &&
			(data.isCgtEligible || data.cgtDiscountApplied)
		) {
			throw new Error(
				"Acquisitions should not be CGT eligible or have CGT discount applied",
			);
		}
		...
		if (
			...
			throw new Error(
				"Income transactions should not be CGT eligible or have CGT discount applied",
			);
		}

		if (
			data.eventType === "NON_TAXABLE" &&
			(data.isCgtEligible || data.cgtDiscountApplied)
		) {
			throw new Error(
				"Non-taxable transactions should not be CGT eligible or have CGT discount applied",
			);
		}
	}
	...
	public getSummaryDescription(): string {
		...
		if (implications.subjectToCgt) {
			const discount = implications.qualifiesForCgtDiscount
				? " (with 50% CGT discount)"
				: "";
	...
	public withCgtDiscount(applied: boolean): TransactionTaxTreatmentModel {
		if (!this.isCgtEligible) {
			throw new Error(
				"Cannot apply CGT discount to non-CGT eligible transaction",
			);
		}
		...
		return new TransactionTaxTreatmentModel({
			...
			treatmentReason: applied
				...
				: this.treatmentReason.replace(
						" - CGT discount applied due to 12+ month holding period",
						"",
					),
	...
	public validate(): {
		...
		if (this.treatmentReason.length < 20) {
			warnings.push(
				"Treatment reason is very short - consider adding more detail",
			);
		}
		...
		if (cgtRules.length > 0 && incomeRules.length > 0) {
			warnings.push(
				"Transaction has both CGT and income rules - ensure treatment is correct",
			);
		}
