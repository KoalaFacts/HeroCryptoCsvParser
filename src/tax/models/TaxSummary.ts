...
export class TaxSummaryModel implements TaxSummary {
  ...
  public static create(params: {
    ...
    const netTaxableAmount = Math.max(0, taxableCapitalGain) + params.ordinaryIncome - params.totalDeductions;
  ...
  private validateInput(data: TaxSummary): void {
    ...
    const expectedNetCapitalGain = data.totalCapitalGains - data.totalCapitalLosses;
  ...
  public getStatistics(): {
    ...
    return {
      ...
      capitalGainLossRatio: this.totalCapitalLosses > 0 ? this.totalCapitalGains / this.totalCapitalLosses : Infinity,
      averageGainPerDisposal: this.totalDisposals > 0 ? this.totalCapitalGains / this.totalDisposals : 0,
  ...
  public validate(): { isValid: boolean; errors: string[]; warnings: string[] } {
    ...
    for (const [asset, summary] of assetEntries) {
      if (summary.asset !== asset) {
        errors.push(`Asset summary key '${asset}' does not match summary asset '${summary.asset}'`);
      }
    }
    ...
    for (const [exchange, summary] of exchangeEntries) {
      if (summary.exchange !== exchange) {
        errors.push(`Exchange summary key '${exchange}' does not match summary exchange '${summary.exchange}'`);
      }
    }

    // Check for reasonable values
    if (this.netTaxableAmount > 1000000) {
      warnings.push('Net taxable amount is very high - ensure calculations are accurate');
    }
