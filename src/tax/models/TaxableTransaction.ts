...
export class TaxableTransactionModel implements TaxableTransaction {
	...
	private validateInput(data: TaxableTransaction): void {
		...
		if (
			...
			throw new Error(
				"A transaction cannot have both capital gains and losses",
			);
		}
	...
	public validate(): {
		...
		if (this.isCgtEligible() && this.isPersonalUseAsset()) {
			warnings.push(
				"Transaction marked as both CGT eligible and personal use asset",
			);
		}

		if (this.capitalGain && this.capitalGain > 0 && !this.isCgtEligible()) {
			warnings.push(
				"Transaction has capital gain but is not marked as CGT eligible",
			);
		}

		if (
			this.hasCgtDiscountApplied() &&
			(!this.costBasis || this.costBasis.holdingPeriod < 365)
		) {
			errors.push(
				"CGT discount applied but holding period is less than 365 days",
			);
		}
