...
export class TaxRuleModel implements TaxRule {
  ...
  public static createAustralianCgtDiscount(): TaxRuleModel {
    return new TaxRuleModel({
      ...
      description: 'Individual taxpayers are entitled to a 50% discount on capital gains from assets held for more than 12 months',
      effectiveFrom: new Date('1999-09-21'), // When CGT discount was introduced
      category: 'CAPITAL_GAINS',
      applicableTransactionTypes: ['SPOT_TRADE', 'SWAP', 'FUTURES_TRADE', 'MARGIN_TRADE'],
      calculationLogic: 'If holdingPeriod >= 365 days and taxpayerType === "INDIVIDUAL" then apply 50% discount to capitalGain'
    });
  }
  ...
  public static createAustralianPersonalUseExemption(): TaxRuleModel {
    return new TaxRuleModel({
      ...
      description: 'Assets acquired for personal use with cost less than $10,000 are exempt from CGT',
      effectiveFrom: new Date('1985-09-20'), // CGT commencement date
      category: 'EXEMPTIONS',
      applicableTransactionTypes: ['SPOT_TRADE', 'SWAP', 'TRANSFER'],
      calculationLogic: 'If assetCost < 10000 AUD and acquiredForPersonalUse === true then exemptFromCGT = true'
    });
  }
  ...
  public static createAustralianStakingIncome(): TaxRuleModel {
    return new TaxRuleModel({
      ...
      description: 'Staking rewards are treated as ordinary income at the time of receipt, valued at fair market value',
      effectiveFrom: new Date('2022-07-01'), // When ATO clarified crypto guidance
      category: 'INCOME',
      applicableTransactionTypes: ['STAKING_REWARD', 'STAKING_DEPOSIT', 'STAKING_WITHDRAWAL'],
      calculationLogic: 'stakingReward.value = marketValueAtReceipt; classifyAs = "ORDINARY_INCOME"'
    });
  }
  ...
  public static createAustralianAirdropIncome(): TaxRuleModel {
    return new TaxRuleModel({
      ...
      description: 'Airdrop tokens are treated as ordinary income when received if they have determinable market value',
      effectiveFrom: new Date('2022-07-01'),
      category: 'INCOME',
      applicableTransactionTypes: ['AIRDROP'],
      calculationLogic: 'If airdrop.marketValue > 0 then classifyAs = "ORDINARY_INCOME"; costBase = marketValueAtReceipt'
    });
  }
  ...
  public static createAustralianDeFiYieldIncome(): TaxRuleModel {
    return new TaxRuleModel({
      ...
      description: 'Yield farming, liquidity mining, and lending rewards are treated as ordinary income when received',
      effectiveFrom: new Date('2022-07-01'),
      category: 'INCOME',
      applicableTransactionTypes: ['LIQUIDITY_ADD', 'LIQUIDITY_REMOVE', 'LOAN', 'INTEREST'],
      calculationLogic: 'If defiReward.type === "YIELD" then classifyAs = "ORDINARY_INCOME"; value = marketValueAtReceipt'
    });
  }
  ...
  public static createAustralianFeeDeduction(): TaxRuleModel {
    return new TaxRuleModel({
      ...
      description: 'Transaction fees can be added to cost base for acquisitions or deducted from proceeds for disposals',
      effectiveFrom: new Date('1985-09-20'),
      category: 'DEDUCTIONS',
      applicableTransactionTypes: ['FEE', 'SPOT_TRADE', 'SWAP', 'TRANSFER'],
      calculationLogic: 'If transactionType === "ACQUISITION" then costBase += fees; else disposalProceeds -= fees'
    });
  }
  ...
  public static createAustralianWashSaleRule(): TaxRuleModel {
    return new TaxRuleModel({
      ...
      description: 'Australia does not have wash sale rules - capital losses can be realized and assets can be immediately repurchased',
      effectiveFrom: new Date('1985-09-20'),
      category: 'EXEMPTIONS',
      applicableTransactionTypes: ['SPOT_TRADE', 'SWAP'],
      calculationLogic: 'washSaleRestrictions = false; allowImmediateRepurchase = true'
    });
  }
  ...
  private validateInput(data: TaxRule): void {
    ...
    if (!data.effectiveFrom || !(data.effectiveFrom instanceof Date) || isNaN(data.effectiveFrom.getTime())) {
      throw new Error('Effective from date must be a valid Date object');
    }

    if (data.effectiveTo) {
      if (!(data.effectiveTo instanceof Date) || isNaN(data.effectiveTo.getTime())) {
        throw new Error('Effective to date must be a valid Date object if provided');
      }
    ...
    if (!Array.isArray(data.applicableTransactionTypes) || data.applicableTransactionTypes.length === 0) {
  ...
  public getActiveDuration(): number {
    ...
    return Math.ceil((endDate.getTime() - this.effectiveFrom.getTime()) / (24 * 60 * 60 * 1000));
  }
  ...
  private hasOverlappingPeriod(other: TaxRule): boolean {
    ...
    const otherEnd = other.effectiveTo ? new Date(other.effectiveTo) : new Date('2100-01-01');
  ...
  public getEffectivePeriodString(): string {
    ...
    const toStr = this.effectiveTo ? this.effectiveTo.toLocaleDateString() : 'present';
  ...
  public validate(): { isValid: boolean; errors: string[]; warnings: string[] } {
    ...
    if (this.calculationLogic.length < 20) {
      warnings.push('Calculation logic is very short - consider adding more detail');
    }

    // Check for standard rule ID format
    if (!this.id.includes('-')) {
      warnings.push('Rule ID should follow format: JURISDICTION-CATEGORY-NUMBER');
    }
