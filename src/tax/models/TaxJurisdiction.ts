...
export class TaxJurisdictionModel implements TaxJurisdiction {
  ...
  private validateInput(data: TaxJurisdiction): void {
    ...
    if (data.code !== 'AU') {
      throw new Error(`Unsupported jurisdiction code: ${data.code}. Only 'AU' is currently supported.`);
    }
    ...
    if (typeof data.cgtDiscountRate !== 'number' || data.cgtDiscountRate < 0 || data.cgtDiscountRate > 1) {
      throw new Error('CGT discount rate must be a number between 0 and 1');
    }

    if (typeof data.cgtHoldingPeriod !== 'number' || data.cgtHoldingPeriod < 0) {
      throw new Error('CGT holding period must be a non-negative number');
    }

    if (typeof data.personalUseThreshold !== 'number' || data.personalUseThreshold < 0) {
      throw new Error('Personal use threshold must be a non-negative number');
    }

    if (!Array.isArray(data.supportedMethods) || data.supportedMethods.length === 0) {
  ...
  private validateTaxYear(taxYear: TaxJurisdiction['taxYear']): void {
    ...
    if (!this.isValidDay(startDay, startMonth) || !this.isValidDay(endDay, endMonth)) {
    ...
    if (startMonth !== 7 || endMonth !== 6) {
      throw new Error('Australian tax year must start in July (month 7) and end in June (month 6)');
    }
  }
  ...
  public getTaxYearStartDate(year: number): Date {
    return new Date(year - 1, this.taxYear.startMonth - 1, this.taxYear.startDay);
  }
