...
import type { TaxSummary, AssetSummary } from './models/TaxSummary';
import { getTransactionAsset, getTransactionTimestamp } from './utils/transactionHelpers';
...
export class TaxSummaryAggregator {
  ...
  generateSummary(transactions: TaxableTransaction[]): TaxSummary {
    ...
    summary.netCapitalGain = summary.totalCapitalGains - summary.totalCapitalLosses;
    summary.taxableCapitalGain = Math.max(0, summary.netCapitalGain - summary.cgtDiscount);
    summary.netTaxableAmount =
      summary.taxableCapitalGain + summary.ordinaryIncome - summary.totalDeductions;
   ...
   */
  private processTransaction(tx: TaxableTransaction, summary: TaxSummary): void {
    // Count event types
    if (tx.taxTreatment.eventType === 'DISPOSAL') {
   ...
   */
  private updateAssetSummary(tx: TaxableTransaction, summary: TaxSummary): void {
   ...
   */
  private updateExchangeSummary(tx: TaxableTransaction, summary: TaxSummary): void {
   ...
   */
  private updateMonthlySummary(tx: TaxableTransaction, summary: TaxSummary): void {
