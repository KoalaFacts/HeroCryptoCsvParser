...
import { TaxReportGenerator, type TaxReportConfig, type ProgressUpdate } from './TaxReportGenerator';
import { TaxOptimizationEngine } from './calculators/TaxOptimizationEngine';
import { exportTaxReportPDF, type PDFExportOptions } from './formatters/PDFReportFormatter';
import { exportTaxReportATO, type ATOExportOptions } from './formatters/ATOXMLFormatter';
import { exportTransactionsToCSV, exportSummaryToCSV, type CSVExportOptions } from './formatters/CSVExporter';
...
export class TaxService {
  ...
  async generateTaxReport(
    ...
    const report = await this.reportGenerator.generateReport(config, onProgress);
   ...
   */
  async getStoredReports(): Promise<Array<{
    id: string;
    jurisdiction: string;
    taxYear: number;
    generatedAt: Date;
  ...
  validateTransactions(
    ...
    warnings: Array<{ code: string; message: string; transaction?: Transaction }>;
  } {
    const errors: Array<{ code: string; message: string; transaction?: Transaction }> = [];
    const warnings: Array<{ code: string; message: string; transaction?: Transaction }> = [];
    ...
    for (const tx of transactions) {
      ...
      if (!tx.type) {
        warnings.push({
          code: 'MISSING_TYPE',
          message: 'Transaction missing type - classification may be inaccurate',
          transaction: tx
        });
      }
    }
