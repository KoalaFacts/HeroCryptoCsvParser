...
export class MMKVAdapter implements StorageAdapter {
  ...
  async update(id: string, updates: Partial<TaxableTransaction>): Promise<void> {
    try {
      ...
      if (!existing) {
        throw new StorageError('Transaction not found', 'NOT_FOUND', 'mobile-native', { id });
      }
  ...
  async listReports(): Promise<TaxReportSummary[]> {
    try {
      ...
      return summaries.sort((a, b) => b.generatedAt.getTime() - a.generatedAt.getTime());
    } catch (error) {
      throw new StorageError(
        'Report list failed',
        'REPORT_LIST_FAILED',
  ...
  private getTransactionKeys(): string[] {
    return this.mmkv.getAllKeys().filter(key => key.startsWith(this.TRANSACTION_PREFIX));
  }

  private getReportKeys(): string[] {
    return this.mmkv.getAllKeys().filter(key => key.startsWith(this.REPORT_PREFIX));
  }
  ...
  private filterKeysByIndices(allKeys: string[], filter: TransactionFilter): string[] {
