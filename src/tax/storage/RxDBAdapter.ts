...
export class RxDBAdapter implements StorageAdapter {
  ...
  constructor(config: StorageConfig) {
    this.config = {
      ...
      indexedFields: config.indexedFields || ['date', 'asset', 'exchange', 'taxEventType']
    };
  }
  ...
  async cleanup(olderThan?: Date): Promise<void> {
    ...
    try {
      const cutoffDate = olderThan || new Date(Date.now() - 365 * 24 * 60 * 60 * 1000); // 1 year ago
      ...
      throw this.createStorageError(
        ...
        'CLEANUP_ERROR',
        { olderThan: olderThan?.toISOString(), originalError: String(error) }
      );
    }
  }
