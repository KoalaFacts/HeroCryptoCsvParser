...
import type { StorageAdapter, StorageConfig as BaseStorageConfig } from './StorageAdapter';
 ...
 */
export type StoragePlatform = 'browser' | 'mobile-native' | 'mobile-web' | 'unified' | 'auto';
...
export class StorageFactory {
  ...
  static createAdapter(config: StorageFactoryConfig = {}): StorageAdapter {
    ...
    const resolvedPlatform = platform === 'auto' ? this.detectPlatform() : platform;

    const baseConfig: BaseStorageConfig = {
      platform: this.mapToPlatform(resolvedPlatform),
      databaseName: config.databaseName || 'crypto-tax-db',
      encryptionKey: config.encryptionKey || (config.encryption ? this.generateEncryptionKey() : undefined),
   ...
   */
  private static mapToPlatform(platform: StoragePlatform): 'browser' | 'mobile-native' | 'mobile-web' | 'node' {
    switch (platform) {
      case 'browser': return 'browser';
      case 'mobile-native': return 'mobile-native';
      case 'mobile-web': return 'mobile-web';
      case 'unified': return 'browser'; // Default unified to browser
  ...
  static detectPlatform(): 'browser' | 'mobile-native' | 'unified' {
    ...
    if (typeof global !== 'undefined' && (global as any).navigator?.product === 'ReactNative') {
  ...
  static isPlatformSupported(platform: StoragePlatform): boolean {
    ...
    switch (platform) {
      ...
      case 'mobile-native':
        return typeof global !== 'undefined' && (global as any).navigator?.product === 'ReactNative';
