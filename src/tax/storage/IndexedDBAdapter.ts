...
export class IndexedDBAdapter implements StorageAdapter {
    ...
    return new Promise((resolve, reject) => {
      ...
      request.onupgradeneeded = (event) => {
        ...
        if (!db.objectStoreNames.contains(this.TRANSACTIONS_STORE)) {
          const txStore = db.createObjectStore(this.TRANSACTIONS_STORE, { keyPath: 'id' });
          txStore.createIndex('date', 'originalTransaction.timestamp', { unique: false });
          txStore.createIndex('asset', 'originalTransaction.asset.symbol', { unique: false });
          txStore.createIndex('exchange', 'originalTransaction.dataSource.name', { unique: false });
          txStore.createIndex('type', 'taxTreatment.eventType', { unique: false });
          txStore.createIndex('taxYear', 'taxYear', { unique: false });
        }

        // Reports store
        if (!db.objectStoreNames.contains(this.REPORTS_STORE)) {
          const reportStore = db.createObjectStore(this.REPORTS_STORE, { keyPath: 'id' });
          reportStore.createIndex('jurisdiction', 'jurisdiction.code', { unique: false });
          reportStore.createIndex('taxYear', 'taxPeriod.year', { unique: false });
  ...
  async batchInsert(transactions: TaxableTransaction[]): Promise<void> {
    if (!this.db) throw new StorageError('Database not initialized', 'DB_NOT_INITIALIZED', 'browser');
    ...
    for (const batch of batches) {
      ...
      if (batchConfig.progressCallback) {
        const processed = batches.indexOf(batch) * batchConfig.batchSize + batch.length;
  ...
  async query(filter: TransactionFilter): Promise<TaxableTransaction[]> {
    ...
    return new Promise((resolve, reject) => {
      ...
      if (filter.dateRange) {
        ...
        const range = IDBKeyRange.bound(filter.dateRange[0], filter.dateRange[1]);
  ...
  async update(id: string, updates: Partial<TaxableTransaction>): Promise<void> {
    if (!this.db) throw new StorageError('Database not initialized', 'DB_NOT_INITIALIZED', 'browser');
    ...
    return new Promise((resolve, reject) => {
      ...
      getRequest.onsuccess = () => {
        ...
        if (!existing) {
          reject(new StorageError('Transaction not found', 'NOT_FOUND', 'browser', { id }));
          return;
        }

        const updated = { ...this.deserializeTransaction(existing), ...updates };
        const serialized = this.serializeTransaction(updated);

        const putRequest = store.put(serialized);
        putRequest.onsuccess = () => resolve();
        putRequest.onerror = () => reject(new StorageError(
          'Update failed',
          'UPDATE_FAILED',
          'browser',
          { id, updates }
      ...
      getRequest.onerror = () => reject(new StorageError(
  ...
  async getReport(id: string): Promise<TaxReport | null> {
    ...
    return new Promise((resolve, reject) => {
      const transaction = this.db!.transaction([this.REPORTS_STORE], 'readonly');
      const store = transaction.objectStore(this.REPORTS_STORE);
  ...
  async getTransactionsByDateRange(start: Date, end: Date): Promise<TaxableTransaction[]> {
  ...
  private matchesFilter(transaction: TaxableTransaction, filter: TransactionFilter): boolean {
    ...
    if (filter.transactionTypes && !filter.transactionTypes.includes(transaction.originalTransaction.type)) {
      return false;
    }

    if (filter.taxEventTypes && !filter.taxEventTypes.includes(transaction.taxTreatment.eventType)) {
      return false;
    }
