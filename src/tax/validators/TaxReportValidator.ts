...
export class TaxReportValidator {
   ...
   * Validate required fields
   */
  private validateRequiredFields(report: TaxReport, issues: ValidationIssue[]): void {
    if (!report.id) {
      issues.push({
        field: 'id',
   ...
   * Validate jurisdiction
   */
  private validateJurisdiction(report: TaxReport, issues: ValidationIssue[]): void {
   ...
   * Validate tax period
   */
  private validateTaxPeriod(report: TaxReport, issues: ValidationIssue[]): void {
   ...
   * Validate transactions
   */
  private validateTransactions(report: TaxReport, issues: ValidationIssue[]): void {
    ...
    for (const tx of report.transactions) {
      ...
      if (txDate < report.taxPeriod.startDate || txDate > report.taxPeriod.endDate) {
        outsidePeriod++;
      }
    }
  ...
  private validateSummary(report: TaxReport, issues: ValidationIssue[]): void {
    ...
    const expectedNetGain = summary.totalCapitalGains - summary.totalCapitalLosses;
  ...
  private validateMetadata(report: TaxReport, issues: ValidationIssue[]): void {
    ...
    if (metadata.totalTransactions !== report.transactions.length) {
      issues.push({
        ...
        message: 'Metadata transaction count does not match actual transactions',
        severity: 'warning',
   ...
   */
  private validateATORequirements(report: TaxReport, issues: ValidationIssue[]): void {
