...
import { getTransactionTimestamp, getTransactionAsset } from '../utils/transactionHelpers';
...
export class ErrorRecovery {
  ...
  recoverMissingCostBasis(
    ...
    const matches = this.findPotentialAcquisitions(disposal, availableAcquisitions);

    if (matches.length > 0) {
      warnings.push(`Found ${matches.length} potential acquisition(s) for matching`);
      return {
        success: true,
    ...
    if (options.allowZeroCostBasis) {
      warnings.push('No cost basis found - using zero (will maximize capital gains)');
      return {
        success: true,
  ...
  private createEstimatedCostBasis(
    ...
    const earliest = matches.reduce((prev, curr) =>
      getTransactionTimestamp(prev) < getTransactionTimestamp(curr) ? prev : curr
    );
  ...
  private areIdentical(tx1: Transaction, tx2: Transaction): boolean {
    return (
      tx1.type === tx2.type &&
      getTransactionTimestamp(tx1).getTime() === getTransactionTimestamp(tx2).getTime() &&
...
export function recoverMissingCostBasis(
  ...
  return recovery.recoverMissingCostBasis(disposal, availableAcquisitions, options);
}
