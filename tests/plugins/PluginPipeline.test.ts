...
describe('PluginRegistry', () => {
  ...
  describe('plugin registration', () => {
    ...
    it('should throw error when registering plugin without name', () => {
      ...
      expect(() => registry.register(plugin)).toThrow('Plugin must have a name');
    });
    ...
    it('should clear all plugins', () => {
      const plugin1: Plugin = { name: 'plugin1', processLine: (ctx, next) => next() };
      const plugin2: Plugin = { name: 'plugin2', processLine: (ctx, next) => next() };
