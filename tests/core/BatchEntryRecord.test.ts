...
class TestRecord extends BatchEntryRecord<TestRecord> {
	...
	constructor() {
		...
		// Field 1: UserName
		this.fieldFor((r) => r.userName, "UserName", 1).mapWith((value) =>
			value.trim(),
		);
		...
		this.fieldFor((r) => r.age, "Age", 2)
			...
			.validateWith((v) =>
				v.min(0, "Age must be positive").max(150, "Age must be realistic"),
			);

		// Field 3: Email (optional)
		this.fieldFor((r) => r.email, "Email", 3).mapWith(
...
describe("BatchEntryRecord", () => {
	...
	describe("parsing", () => {
		it("should parse valid CSV line", () => {
			const result = BatchEntryRecord.parse(
				TestRecord,
				"123,John Doe,30,john@example.com,extra",
			);
		...
		it("should validate required fields", () => {
			const result = BatchEntryRecord.parse(
				TestRecord,
				",John Doe,30,john@example.com,extra",
			);
		...
		it("should validate numeric ranges", () => {
			const result = BatchEntryRecord.parse(
				TestRecord,
				"123,John Doe,200,john@example.com,extra",
			);
