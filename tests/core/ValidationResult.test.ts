...
describe('ValidationResult', () => {
  ...
  describe('addErrors', () => {
    it('should add multiple errors at once', () => {
      ...
      expect(result.errors.map(e => e.code)).toEqual(['ERROR1', 'ERROR2', 'ERROR3']);
    });
  ...
  describe('static methods', () => {
    ...
    describe('combine', () => {
      it('should combine multiple results', () => {
        ...
        const result2 = ValidationResult.failure({ code: 'ERROR1', message: 'Error 1' });
        const result3 = ValidationResult.failure({ code: 'ERROR2', message: 'Error 2' });

        const combined = ValidationResult.combine([result1, result2, result3]);

        expect(combined.isValid).toBe(false);
        expect(combined.errors).toHaveLength(2);
        expect(combined.errors.map(e => e.code)).toEqual(['ERROR1', 'ERROR2']);
      });
