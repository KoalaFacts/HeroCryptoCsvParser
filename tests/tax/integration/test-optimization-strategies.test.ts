...
interface TaxOptimizer {
  analyzeTaxPosition(transactions: Transaction[], taxYear: string): Promise<{
  ...
  optimizeDisposalTiming(
    ...
    plannedDisposals: { asset: string, amount: number, urgency: 'low' | 'medium' | 'high' }[]
  ): Promise<CGTOptimization[]>;
  calculateWashSaleImpact(
    saleTransaction: Transaction,
    repurchaseTransaction: Transaction
  ): Promise<{ isWashSale: boolean, disallowedLoss: number, adjustedCostBasis: number }>;
