...
import { CGTDiscountRules, PersonalUseAssetRules } from '@/tax/rules/AustralianTaxRules';
import { TransactionValidator, TaxReportValidator, ATOFormatValidator } from '@/tax/validators';
...
describe('Tax Module - All Unit Tests', () => {
  describe('FIFOCalculator', () => {
    it('should calculate cost basis using FIFO', () => {
      ...
      const acq = createMockSpotTrade({ id: 'acq1', timestamp: new Date('2024-01-01') });
      const disposal = createMockSpotTrade({ id: 'disp1', timestamp: new Date('2024-06-01'), side: 'SELL' });
  ...
  describe('Personal Use Asset Rules', () => {
    ...
    it('should not classify as personal use when >=$10k', () => {
      const result = PersonalUseAssetRules.isPersonalUseAsset(10000, 'PERSONAL');
      expect(result).toBe(false);
    });

    it('should return 0 taxable for exempt gains', () => {
      const taxable = PersonalUseAssetRules.calculateTaxableAmount(5000, 5000, 'PERSONAL');
