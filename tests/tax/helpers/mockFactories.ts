 ...
 */
export function createMockAsset(symbol: string, options?: {
  name?: string;
  decimals?: number;
 ...
 */
export function createMockDataSource(id: string, type: 'exchange' | 'wallet' | 'defi' | 'blockchain' | 'manual' | 'protocol' | 'marketplace' = 'exchange', name?: string): DataSource {
 ...
 */
export function createMockFiatValue(value: string, currency: string = 'USD'): FiatValue {
  return {
...
export function createMockAssetAmount(
  ...
  return {
    ...
    amount: createMockAmount(value),
    fiatValue: fiatValue ? createMockFiatValue(fiatValue.value, fiatValue.currency) : undefined
  };
}
...
export function createMockSpotTrade(overrides?: Partial<SpotTrade>): SpotTrade {
  const defaults: SpotTrade = {
    ...
    baseAsset: createMockAssetAmount('BTC', '0.5', { value: '25000', currency: 'USD' }),
    quoteAsset: createMockAssetAmount('USDT', '25000', { value: '25000', currency: 'USD' }),
    side: 'BUY',
    price: '50000',
 ...
 */
export function createMockStakingReward(overrides?: Partial<StakingReward>): StakingReward {
  const defaults: StakingReward = {
    id: 'test-staking-001',
    type: 'STAKING_REWARD',
    timestamp: new Date('2024-01-15T10:30:00Z'),
    source: createMockDataSource('binance', 'exchange', 'Binance'),
    reward: createMockAssetAmount('ETH', '0.05', { value: '100', currency: 'USD' }),
    staking: {
...
export function createMockSwap(overrides?: Partial<Swap>): Swap {
  const defaults: Swap = {
    ...
    from: createMockAssetAmount('USDC', '1000', { value: '1000', currency: 'USD' }),
...
export function createMockTransfer(overrides?: Partial<Transfer>): Transfer {
  const defaults: Transfer = {
    ...
    asset: createMockAssetAmount('ETH', '1.0', { value: '2000', currency: 'USD' }),
    from: { address: '0x123...', label: 'Wallet 1' },
    to: { address: '0x456...', label: 'Wallet 2' },
    direction: 'OUT',
    networkFee: createMockAssetAmount('ETH', '0.001', { value: '2', currency: 'USD' }),
    taxEvents: []
  };
...
export function createMockAirdrop(overrides?: Partial<Airdrop>): Airdrop {
  const defaults: Airdrop = {
    ...
    timestamp: new Date('2024-01-15T10:30:00Z'),
    source: createMockDataSource('uniswap', 'defi', 'Uniswap'),
    received: createMockAssetAmount('UNI', '400', { value: '2000', currency: 'USD' }),
    airdrop: {
...
export function createMockInterest(overrides?: Partial<Interest>): Interest {
  const defaults: Interest = {
    ...
    interest: createMockAssetAmount('USDC', '10', { value: '10', currency: 'USD' }),
    interestType: 'EARNED',
    context: {
      protocol: 'Aave',
      rate: '3.5',
 ...
 */
export function createMockLiquidityAdd(overrides?: Partial<LiquidityAdd>): LiquidityAdd {
 ...
 */
export function createMockLiquidityRemove(overrides?: Partial<LiquidityRemove>): LiquidityRemove {
 ...
 */
export function createMockTransaction(type: Transaction['type'], overrides?: any): Transaction {
  switch (type) {
    case 'SPOT_TRADE':
...
export const TestAssets = {
  ...
  ETH: createMockAsset('ETH', { name: 'Ethereum', decimals: 18, type: 'crypto' }),
  USDT: createMockAsset('USDT', { name: 'Tether', decimals: 6, type: 'stablecoin' }),
  USDC: createMockAsset('USDC', { name: 'USD Coin', decimals: 6, type: 'stablecoin' }),
  BNB: createMockAsset('BNB', { name: 'Binance Coin', decimals: 8, type: 'crypto' }),
  DAI: createMockAsset('DAI', { name: 'Dai', decimals: 18, type: 'stablecoin' }),
