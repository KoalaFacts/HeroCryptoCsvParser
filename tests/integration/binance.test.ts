...
import { process, exportToCSV, CsvExportOptions } from '@/index';
import {
  Transaction,
  SpotTrade,
  Transfer,
...
"123456789","2024-03-03 00:00:00","Earn","Liquid Swap remove","BUSD","1000.12340000","Remove liquidity from pool"`;
      ...
      expect((liquidityReward as any).originalData?.categorization?.subType).toBe('liquidity');
...
"123456789","2024-01-15 12:00:00","OTC","OTC Trading","ETH","2.00000000","OTC Desk Trading"`;
      ...
      expect((p2pBuy as any).originalData?.categorization?.subType).toBe('p2p-buy');

      const p2pSell = result.transactions[1] as SpotTrade;
      expect(p2pSell.type).toBe('SPOT_TRADE');
      expect((p2pSell as any).originalData?.categorization?.subType).toBe('p2p-sell');

      const otc = result.transactions[2] as SpotTrade;
      expect(otc.type).toBe('SPOT_TRADE');
      expect((otc as any).originalData?.categorization?.subType).toBe('otc');
    });
...
"123456789","2024-03-01 04:00:00","Earn","Auto-Invest","BTC","-0.00100000","DCA Investment"`;
      ...
      expect((flexInterest as any).originalData?.categorization?.subType).toBe('flexible-savings');
      ...
      expect((launchpool as any).originalData?.categorization?.subType).toBe('launchpool');

      // Savings interest
      const savings = result.transactions[3] as Interest;
      expect(savings.type).toBe('INTEREST');
      expect((savings as any).originalData?.categorization?.subType).toBe('savings');

      // Auto-invest should be swap
      const autoInvest = result.transactions[4] as Swap;
      expect(autoInvest.type).toBe('SWAP');
      expect((autoInvest as any).originalData?.categorization?.subType).toBe('auto-invest');
    });
...
"123456789","2024-02-01 15:00:00","COIN-Futures","Insurance Clear","BTC","-0.00010000","Insurance Fund"`;
      ...
      expect((marginInterest as any).originalData?.categorization?.subType).toBe('margin-interest');
      ...
      expect((unrealizedPnl as any).originalData?.categorization?.subType).toBe('unrealized-pnl');

      // Insurance clear
      const insurance = result.transactions[5] as Fee;
      expect(insurance.type).toBe('FEE');
      expect((insurance as any).originalData?.categorization?.subType).toBe('insurance');
    });
...
"123456789","2024-03-01 05:00:00","Spot","Cash Voucher Distribution","BUSD","20.00000000","Promotional Voucher"`;
      ...
      expect((result.transactions[0] as any).originalData?.categorization?.subType).toBe('vip');
      expect((result.transactions[1] as any).originalData?.categorization?.subType).toBe('task');
      expect((result.transactions[2] as any).originalData?.categorization?.subType).toBe('competition');
      expect((result.transactions[3] as any).originalData?.categorization?.subType).toBe('event');
      expect((result.transactions[4] as any).originalData?.categorization?.subType).toBe('affiliate');
      expect((result.transactions[5] as any).originalData?.categorization?.subType).toBe('voucher');
    });
...
"123456789","2024-01-21 11:00:00","Spot","Dust to BNB","FLOKI","-50000.00000000","Dust Conversion"`;
      ...
      result.transactions.forEach(tx => {
        ...
        expect((swap as any).originalData?.categorization?.subType).toBe('dust');
      });
    });
  });
...
"123456789","2024-01-17 16:15:00","Spot","Auto-Invest","BTC","-0.00050000","DCA Investment Strategy"`;
      ...
      const exportOptions: CsvExportOptions = {
        ...
        fields: [
          ...
          'asset', 'amount',
          'baseAsset', 'baseAmount', 'quoteAsset', 'quoteAmount', 'side', 'price',
          'direction', 'transferType', 'from', 'to',
          'feeAsset', 'feeAmount', 'feeType',
          'fromAsset', 'fromAmount', 'toAsset', 'toAmount',
          'stakingProtocol', 'stakingAPR', 'lockupPeriod',
...
"987654321","2024-03-04 12:30:00","Spot","Cash Voucher Distribution","BUSD","50.00000000","Promotional Campaign Reward"`;
      ...
      const tradingAnalysisExport = exportToCSV(result.transactions, {
        ...
        fields: ['timestamp', 'type', 'asset', 'amount', 'description', 'taxable']
      });
      ...
      expect(headerLine).toBe('timestamp,type,asset,amount,description,taxable');
      ...
      const transactionTypes = new Set(result.transactions.map(tx => tx.type));
...
"555555555","2024-02-16 12:15:00","Earn","Auto-Invest","BTC","-0.00100000","Dollar-cost averaging"`;
      ...
      const p2pTrade = result.transactions.find(tx =>
        (tx as any).originalData?.categorization?.subType === 'p2p-buy'
      );
      expect(p2pTrade).toBeDefined();
      expect(p2pTrade?.type).toBe('SPOT_TRADE');

      const vipReward = result.transactions.find(tx =>
        (tx as any).originalData?.categorization?.subType === 'vip'
      );
